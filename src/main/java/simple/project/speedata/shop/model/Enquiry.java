...
public class Enquiry extends BaseModel implements Serializable {
	...
	@RepresentationField(sort = 10, title = "项目名称", isSearchField = true)
	@TableColumn(title = "项目名称")
	@NotNull(message = "项目名称不能为空！")
	@Length(max = DataLength.LONG_TEXT_LENGTH)
	private String project;

	@Column(name = "FILE_NAME", length = DataLength.MAX_STRING_LENGTH)
	@RepresentationField(sort = 10, title = "功能描述", isSearchField = true, view = RepresentationFieldType.TEXTAREA)
	...
	@RepresentationField(title = "是否定制", sort = 99995, view = RepresentationFieldType.BOOLEAN, defaultVal = "true")
	@BooleanValue({ "是", "否" })
	@Convert(converter = BooleanToStringConverter.class)
	private Boolean isCustom;

	@Column(name = "EMAIL", length = DataLength.LONG_TEXT_LENGTH)
	@RepresentationField(sort = 10, title = "邮箱", isSearchField = true)
